...
import { TEditService, TSaveSortedService, TService } from "@/types/types";
import { editServiceSchema } from "@/types/schema";
import {
  Form,
  FormControl,
  FormField,
...
import { useEditService, useSaveSortedService } from "@/service/mutation";
...
import { useGetAllServices, useGetService } from "@/service/queries";
...
const EditService = () => {
  ...
  const { data: serviceData, isFetched } = useGetService(id!);
  const currentService: TService | undefined = serviceData?.data[0];
  ...
  const form = useForm<TEditService>({
  ...
  useEffect(() => {
    ...
    if (isFetched && currentService) {
      form.setValue("title", currentService.title);
      form.setValue("description", currentService.description);
      form.setValue("visible", currentService.visible);
    }
  }, [
    form,
    id,
  ...
  const handleEditService: SubmitHandler<TEditService> = async (data) => {
    await editService.mutate(data);
  };
  return (
    <div>
      ...
      <div className="flex gap-4 mt-4">
        ...
        <Card className="w-[70%]">
          <CardHeader>Edit Service</CardHeader>
          <CardContent>
            <Form {...form}>
              <form
